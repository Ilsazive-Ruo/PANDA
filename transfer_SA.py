...
def load_SASet(file, g_dict):
    ...
    for index in dataset_file.index:
        if dataset_file['API'][index] in g_dict.keys() and dataset_file['excipient'][index] in g_dict.keys():
            ...
            labels.append(dataset_file['label'][index])
...
if __name__ == '__main__':
    ...
    graph_pairs, labels = load_SASet(args['set_file'], g_dict)
    ...
    model = module2.SA(pre_model=pre_model, fusion_feature_size=node_feat_size)
    ...
    torch.save(model, 'models/SA/model.pth')


# python transfer_SA.py -g data/gra/Sa/SA_dgl_graph.bin -d data/gra/SA/SA_info.pkl -s data/data_set.csv -b 128 -e 300
